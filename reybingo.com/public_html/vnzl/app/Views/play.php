...
<head>
  ...
  <script>
    ...
    const API_JOIN   = <?= json_encode(site_url("api/games/{$gameId}/join")) ?>;
    const API_CLAIM  = <?= json_encode(site_url("api/games/{$gameId}/claim-bingo")) ?>;
...
<body>
  <h1 style="text-align: center; color: #333;">🎯 BINGO - Juego <?= esc($gameId) ?></h1>
  ...
  <script>
    ...
    // ---- ids ----
    const playerId = (crypto && crypto.randomUUID) ? crypto.randomUUID()
